namespace BillsFrontEndBlazor.Models
{
    ...
    /// <summary>
    /// Window arithmetic for <see cref="ReportRange"/>, shared by the Reports
    /// page and the CSV endpoint so an export can never cover a different set of
    /// bills than the page it was downloaded from.
    /// </summary>
    public static class ReportRanges
    {
        ...
        public static List<Bill> Filter(IEnumerable<Bill> bills, ReportRange range, DateTime today) =>
            bills.Where(b => range.Includes(today, b.DueDate)).ToList();
